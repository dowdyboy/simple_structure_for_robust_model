...
def get_LID(model, test_clean_data, test_adv_data, test_noisy_data, test_label, num_output):
    ...
    for data_index in range(int(np.floor(test_clean_data.size(0)/batch_size))):
        ...
        for i in range(num_output):
            ...
            X_act.append(np.asarray(out_features[i].detach().cpu(), dtype=np.float32).reshape((out_features[i].size(0), -1)))
        ...
        for i in range(num_output):
            ...
            X_act_adv.append(np.asarray(out_features[i].detach().cpu(), dtype=np.float32).reshape((out_features[i].size(0), -1)))
        ...
        for i in range(num_output):
            ...
            X_act_noisy.append(np.asarray(out_features[i].detach().cpu(), dtype=np.float32).reshape((out_features[i].size(0), -1)))
...
def get_test_LID(model, test_clean_data, test_label, num_output):
    '''
    Compute LID score on adversarial samples
    return: LID score
    '''
...
def get_single_LID(model, test_clean_data, test_label, num_output):
    '''
    Compute LID score on adversarial samples
    return: LID score
    '''
    ...
    data = torch.unsqueeze(test_clean_data, dim=0).cuda()
    target = torch.unsqueeze(torch.tensor(test_label), dim=0).cuda()
...
def get_Mahalanobis_score_adv_single(model, test_data, test_label, num_classes, outf, net_type, sample_mean, precision, layer_index, magnitude):
    '''
    Compute the proposed Mahalanobis confidence score on adversarial samples
    return: Mahalanobis score from layer_index
    '''
    ...
    target = torch.unsqueeze(torch.tensor(test_label), dim=0).cuda()
    data = torch.unsqueeze(test_data, dim=0).cuda()
