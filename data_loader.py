...
def getTargetDataSet(data_type, batch_size, input_TF, dataroot):
    ...
    elif data_type == 'aisafe':
        train_dir = os.path.join(dataroot, 'train')
        val_dir = os.path.join(dataroot, 'val')
        train_dataset = ImageFolder(train_dir, transform=input_TF, )
        val_dataset = ImageFolder(val_dir, transform=input_TF, )
        train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=1, )
        test_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False, num_workers=1, )
